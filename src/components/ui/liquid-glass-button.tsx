...
// ─── Liquid Glass SVG Filter (render ONCE per page) ──────────────────────────
// Place <LiquidGlassFilter /> once near the root of your layout / Navbar.
// All LiquidButton instances reference it by its id.
...
const liquidbuttonVariants = cva(
  "relative inline-flex items-center justify-center cursor-pointer gap-2 whitespace-nowrap rounded-full text-sm font-medium overflow-hidden transition-[transform,filter] duration-200 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:ring-2 focus-visible:ring-white/50",
  {
    variants: {
      variant: {
        default: "text-white hover:scale-[1.04] active:scale-[0.97]",
...
function LiquidButton({
  ...
  return (
    <Comp
      ...
      {...props}
    >
      {/* Layer 1 — backdrop blur (cross-browser: Chrome, Safari, Firefox) */}
      <span
        aria-hidden="true"
      ...
      {/* Layer 2 — glass tint + edge highlights */}
      <span
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 rounded-[inherit] bg-white/[0.08] border border-white/25"
        style={{
          boxShadow:
            "inset 0 1px 1px rgba(255,255,255,0.35), inset 0 -1px 1px rgba(0,0,0,0.18)",
        }}
      />

      {/* Layer 3 — outer raised-bubble shadow */}
      <span
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 rounded-[inherit] transition-all duration-200"
        style={{
          boxShadow:
            "0 2px 8px rgba(0,0,0,0.18), 0 0 0 1px rgba(255,255,255,0.08), inset 1px 1px 1px rgba(255,255,255,0.5), inset -1px -1px 1px rgba(255,255,255,0.2)",
        }}
      />

      {/* Layer 4 — SVG distortion (Firefox only, enhances; others ignore gracefully) */}
      <span
        aria-hidden="true"
